/*Ordenación por el método de montículo (heapshort)*/

/*ALGORITMO ADAPTADO AL BENCHMARK*/

   void criba(int izq, int dcha, int *lista,Experiment * experiment){
      int i, doble,aux;
      aux=lista[izq];
...
/*Algoritmo para pruebas sin el resto de programa*/
  /* void criba(int izq, int dcha, int *lista){
      int i, doble,aux;
      aux=lista[izq];
      for(i=izq; doble= 2*i+1,doble<=dcha; i =doble){
        if(doble<dcha && lista[doble] < lista[doble +1]){
           doble++;
        }
        if(aux<lista[doble]){
           lista[i]=lista[doble];
   ...
   void monticulo( int *lista, int N){
      int i,aux;
      for(i=N/2 -1;i>=0;i--){
         criba(i,N-1,lista);
      }
